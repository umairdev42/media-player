...
heading_label = tk.Label(root, text="Audio Player", font=("Arial", 18, "bold"))
...
def add_song():
    song = filedialog.askopenfilename(initialdir='audio/', title="Choose A Song", filetypes=(("mp3 Files", "*.mp3;*.wav;*.ogg" ), ))
...
def add_many_songs():
    songs = filedialog.askopenfilenames(initialdir='audio/', title="Choose A Song", filetypes=(("mp3 Files", "*.mp3;*.wav;*.ogg" ), ))
...
# Create Function To Search and Highlight Song in Playlist
def search_song():
    query = search_entry.get().lower()  # Get the search query and convert to lowercase for case-insensitive matching
    for i, song in enumerate(playlist_box.get(0, END)):
        if query in song.lower():
            playlist_box.selection_clear(0, END)
            playlist_box.selection_set(i)
            playlist_box.activate(i)
            break
...
# Create Playlist Box
playlist_box = Listbox(main_frame, bg="white", fg="black", width=180, height=20, selectbackground="green", selectforeground='white')
playlist_box.grid(row=0, column=0)

# Create volume slider frame
volume_frame = LabelFrame(main_frame, text="Volume")
volume_frame.grid(row=2, column=1, padx=20)

# Create Volume Slider
volume_slider = ttk.Scale(volume_frame, from_=0, to=1, orient=HORIZONTAL, length=125, value=1, command=volume)
volume_slider.pack(pady=10)

# Create Song Slider
song_slider = ttk.Scale(main_frame, from_=0, to=100, orient=HORIZONTAL, length=960, value=0, command=slide)
song_slider.grid(row=3, column=0, pady=10)
...
search_button = Button(search_frame, text="Search", command=search_song, bg="red", fg="white", width=10)
